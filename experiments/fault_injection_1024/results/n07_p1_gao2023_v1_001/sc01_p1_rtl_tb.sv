`timescale 1ns/1ps
// Isolated P1 inject TB: SYMBOL_MIN/MAX like S3. Clean = original P1 top.
module sc01_p1_fault_injection_tb;
parameter integer THRESHOLD    = 0;
parameter integer STAGE_MODE   = 0;
parameter integer SYMBOL_MIN   = 0;
parameter integer SYMBOL_MAX   = 5;
localparam integer N_SYMBOLS   = SYMBOL_MAX - SYMBOL_MIN + 1;
localparam integer TRIALS_ST17 = 7 * N_SYMBOLS * 70;
localparam integer TRIALS_S10  = N_SYMBOLS * 70;
...
initial begin
 ...
 $display("SC01_P1_RTL threshold=%0d stage_mode=%0d symbols=%0d..%0d total_trials=%0d (st17=%0d,s10=%0d)",THRESHOLD,STAGE_MODE,SYMBOL_MIN,SYMBOL_MAX,TOTAL_TRIALS,TRIALS_ST17,TRIALS_S10);
 trial=0;
   ...
   for(symbol_now=SYMBOL_MIN;symbol_now<=SYMBOL_MAX;symbol_now=symbol_now+1) begin
    for(component_now=0;component_now<=1;component_now=component_now+1) begin
     for(bit_now=0;bit_now<35;bit_now=bit_now+1) begin
      run_one_trial(stage_now[3:0],symbol_now[2:0],component_now[0:0],bit_now[5:0]);
      trial=trial+1;
     end
    end
   end
 ...
 if(STAGE_MODE!=1) begin
  for(symbol_now=SYMBOL_MIN;symbol_now<=SYMBOL_MAX;symbol_now=symbol_now+1) begin
   for(component_now=0;component_now<=1;component_now=component_now+1) begin
    for(bit_now=0;bit_now<35;bit_now=bit_now+1) begin
     run_one_trial(4'd10,symbol_now[2:0],component_now[0:0],bit_now[5:0]);
     trial=trial+1;
    end
   end
  end
