#!/usr/bin/env bash
# N07 P1 Gao-2023 inject. Usage: bash run_inject.sh <THRESHOLD>
set -euo pipefail
...
: > "$PROGRESS"
: > "$COMBINED"
echo "$(date -Iseconds) START N07 P1 tau=$TAU stages=1-7+10 symbols=0-5 two-frames" | tee -a "$PROGRESS"
...
echo "$(date -Iseconds) ALL_DONE P1 tau=$TAU 3360 trials" | tee -a "$PROGRESS"
