`timescale 1ns/1ps
// Fault-free raw-syndrome dump for Gao 2023 3-sigma calibration on K_P1.
module tb_p1_gao2023_noise_floor;
  ...
  initial begin
    ...
    if (!$value$plusargs("INPUT_HEX=%s", input_hex))
      input_hex = "/home/xdu/JuWade_research/fft1024_ft_exp/kernels/P1/vectors/qualification_input_10frames.hex";
    if (!$value$plusargs("EXPECTED_HEX=%s", expected_hex))
      expected_hex = "/home/xdu/JuWade_research/fft1024_ft_exp/kernels/P1/vectors/qualification_pfft_expected_8frames.hex";
  ...
  always @(negedge clk) if (!rst) begin
    ...
    dump_stage(10, dut.u.s10.go_syn, dut.u.s10.sy0_ur, dut.u.s10.sy0_ui, dut.u.s10.sy1_ur, dut.u.s10.sy1_ui, dut.u.s10.found_r);
    if (out_valid) begin
      if (output_index < EXPECTED_BEATS) begin
        if (packed_output !== expected_mem[output_index]) begin
          errors = errors + 1;
          if (errors <= 5) $display("P1 MISMATCH beat=%0d", output_index);
        end
