...
localparam integer EXPECTED_LATENCY=`PROJECT_EXPECTED_LATENCY;
...
integer input_index=0,output_index=0,cycle_count=0,first_input=-1,first_output=-1;
integer errors=0,gap_errors=0,last_errors=0,data_errors=0;
reg output_started=0;
`PROJECT_TOP dut(clk,rst,in_valid,in_last,in0_re,in0_im,in1_re,in1_im,in2_re,in2_im,in3_re,in3_im,
...
initial begin
 ...
 $readmemh(`PROJECT_EXPECTED_FILE,expected_mem);
...
always@(posedge clk)if(!rst)begin
 ...
 if(output_started&&(output_index<EXPECTED_BEATS)&&!out_valid)begin
  ...
  if(gap_errors<=8)$display("OUT_VALID_GAP architecture=%s cycle=%0d after_beat=%0d",`PROJECT_ID,cycle_count,output_index-1);
 end
 if(out_valid)begin
  ...
  if(output_index<EXPECTED_BEATS)begin
   if(packed_output!==expected_mem[output_index])begin
    ...
    if(data_errors<=8)$display("MISMATCH architecture=%s beat=%0d",`PROJECT_ID,output_index);
   end
   if(out_last!==((output_index%256)==255))begin
    errors=errors+1;last_errors=last_errors+1;
    if(last_errors<=8)$display("OUT_LAST_ERROR architecture=%s beat=%0d value=%0b",`PROJECT_ID,output_index,out_last);
   end
   output_index=output_index+1;
   if(output_index==EXPECTED_BEATS)begin
    if((first_output-first_input)!=EXPECTED_LATENCY)begin
     errors=errors+1;
     $display("LATENCY_ERROR architecture=%s actual=%0d expected=%0d",`PROJECT_ID,first_output-first_input,EXPECTED_LATENCY);
    end
    if(errors==0)begin
     $display("PASS_V3 architecture=%s beats=%0d latency=%0d gaps=0 last_errors=0 data_errors=0",
      `PROJECT_ID,output_index,first_output-first_input);
     $finish;
    end else $fatal(1,"FAIL_V3 architecture=%s errors=%0d data=%0d gaps=%0d last=%0d",
     `PROJECT_ID,errors,data_errors,gap_errors,last_errors);
   end
  end else begin
   errors=errors+1;
   $fatal(1,"EXTRA_OUTPUT architecture=%s cycle=%0d",`PROJECT_ID,cycle_count);
  end
 end
