#!/usr/bin/env python3
"""Isolated author-approved attempt2 Yosys evidence flow for PFFT-RES-V3-001.
...
* ``--validate-only``
* ``--preflight``
* ``--preflight --attempt attempt2``
* ``--synthesize --attempt attempt2``
...
RESULTS = BASE_RESULTS / "attempt2"
LOGS = EXPERIMENT / "logs" / "pfft_resource_v3_001" / "attempt2"
BUILD = EXPERIMENT / "build" / "pfft_resource_v3_001" / "attempt2"

VALIDATION_RESULT = BASE_RESULTS / "yosys_input_validation.json"
PREFLIGHT_RESULT = RESULTS / "yosys_preflight.json"
RAW_RESULT = RESULTS / "yosys_raw_measurements_attempt2.json"
...
def run_synthesis(
    ...
    if attempt != "attempt2":
        raise GateFailure(
            "runner",
            "PFFT-RES-V3-001 S06 isolated runner authorizes only --attempt attempt2",
        )
    prior = require_prior_validation(config, fresh_validation)
    ...
    if (
        ...
        raise GateFailure(
            "synthesis",
            "attempt2 or final artifacts already exist; refusing an implicit rerun",
        )
    directory.mkdir(parents=True)
    rows: list[dict[str, Any]] = []
    try:
...
def main() -> int:
    ...
    if args.attempt != "attempt2":
        print("STOP runner: only --attempt attempt2 is authorized", flush=True)
        return 2
    try:
