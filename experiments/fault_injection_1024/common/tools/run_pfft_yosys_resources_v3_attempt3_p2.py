#!/usr/bin/env python3
"""Isolated author-approved attempt3 P2-only Yosys flow for PFFT-RES-V3-001.
...
* ``--validate-only``
* ``--preflight``
* ``--preflight --attempt attempt3``
* ``--synthesize --attempt attempt3``
...
CANONICAL_RUNNER = HERE / "run_pfft_yosys_resources_v3.py"
...
BASE_RESULTS = EXPERIMENT / "results" / "pfft_resource_v3_001"
RESULTS = BASE_RESULTS / "attempt3_p2"
LOGS = EXPERIMENT / "logs" / "pfft_resource_v3_001" / "attempt3_p2"
BUILD = EXPERIMENT / "build" / "pfft_resource_v3_001" / "attempt3_p2"

VALIDATION_RESULT = BASE_RESULTS / "yosys_input_validation.json"
PREFLIGHT_RESULT = RESULTS / "yosys_preflight.json"
RAW_RESULT = RESULTS / "yosys_raw_measurements_attempt3_p2.json"
...
def validate_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    fingerprint = {
        ...
        "runner_sha256": sha256(CANONICAL_RUNNER),
...
def run_preflight(
    ...
    expected_logs = [
        ...
        for architecture_id in RUN_ARCH_ORDER
    ]
    if (
        directory.exists()
...
def run_synthesis(
    ...
    if attempt != "attempt3":
        raise GateFailure(
            "runner",
            "PFFT-RES-V3-001 S06 isolated P2 runner authorizes only --attempt attempt3",
        )
    prior = require_prior_validation(config, fresh_validation)
    ...
    if (
        ...
        raise GateFailure(
            "synthesis",
            "attempt3 P2 or final artifacts already exist; refusing an implicit rerun",
        )
    directory.mkdir(parents=True)
    rows: list[dict[str, Any]] = []
    try:
                ...
                write_json(
                    RAW_RESULT,
                    {
                        ...
                        "unexecuted_architectures": list(
                            RUN_ARCH_ORDER[RUN_ARCH_ORDER.index(architecture_id) + 1 :]
                        ),
                        "failure_policy": (
        ...
        write_json(
            ...
            {
                "schema": "pfft-resource-v3-001-yosys-raw-measurements-v1",
                "status": "VERIFIED_P2_COMPLETION",
                "result_class": "VERIFIED_PARTIAL_ARCHITECTURE_COMPLETION",
                "timestamp_utc": utc_now(),
                "experiment_id": "PFFT-RES-V3-001",
                "attempt": attempt,
                "evidence_class": "raw_Yosys_xc7_synthesis_measurements",
                ...
                json.dumps(
                    {
                        ...
                        "raw": display_path(RAW_RESULT),
                        "raw_sha256": sha256(RAW_RESULT),
                        "policy": "no_cross_attempt_final_table_generated",
                    },
                    ensure_ascii=False,
                ),
                flush=True,
...
def main() -> int:
    ...
    if args.attempt != "attempt3":
        print("STOP runner: only --attempt attempt3 is authorized", flush=True)
        return 2
    try:
