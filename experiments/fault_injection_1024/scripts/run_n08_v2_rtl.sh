...
if [[ "$ARCH" == s3 ]]; then
  OUT="$ROOT/experiments/fault_injection_1024/results/n08_multi_k${K}_s3_v2_001"
  ...
  OUT="$ROOT/experiments/fault_injection_1024/results/n08_multi_k${K}_p1_v2_001"
...
SITE_ARG="${SITE#$ROOT/}"
SIM="/tmp/n08_v2_${ARCH}_k${K}.vvp"
LOG="$OUT/run_k${K}.log"
PROGRESS="$OUT/progress.txt"
COMPILE="$OUT/compile.log"

echo "$(date -Iseconds) START_V2 arch=$ARCH k=$K trials=$TRIALS tau=$TAU site=$SITE_ARG" | tee "$PROGRESS"
...
echo "$(date -Iseconds) COMPILE_OK_V2 arch=$ARCH k=$K" | tee -a "$PROGRESS"
...
while kill -0 "$PID" 2>/dev/null; do
  ...
  echo "$(date -Iseconds) RUNNING_V2 arch=$ARCH k=$K trials=$count/$TRIALS last=${last:-none}" | tee -a "$PROGRESS"
  sleep 15
done
wait "$PID"
summary=$(grep '^SUMMARY ' "$LOG" | tail -1 || true)
echo "$(date -Iseconds) DONE_V2 arch=$ARCH k=$K $summary" | tee -a "$PROGRESS"
