...
top_p0_pfft_no_framebuf_v1 dut(
...
initial begin
 $readmemh(
  ...
  input_mem);
 $readmemh(
  "experiments/fault_injection_1024/projects/P0/vectors/qualification_p0_no_exchange_expected_8frames.hex",
  expected_mem);
...
always@(posedge clk)if(!rst)begin
 ...
 if(out_valid)begin
  ...
  if(frame_beat_count==255)begin
   ...
   frame_beat_count=frame_beat_count+1;
  end
  output_index=output_index+1;
  if(output_index==EXPECTED_BEATS)begin
   if(frame_count!=8||frame_beat_count!=0)begin
    errors=errors+1;
    $display("FRAME_BOUNDARY_ERROR frames=%0d residual_beats=%0d",
     frame_count,frame_beat_count);
   end
   if(errors==0)begin
    $display(
     "PASS_NO_FRAMEBUF beats=%0d frames=%0d measured_latency=%0d gaps=0 last_errors=0 data_errors=0",
     output_index,frame_count,first_output-first_input);
    $finish;
   end else begin
    $fatal(1,
     "FAIL_NO_FRAMEBUF errors=%0d data=%0d gaps=%0d last=%0d frames=%0d",
